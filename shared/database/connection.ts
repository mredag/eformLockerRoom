...
export class DatabaseConnection {
  ...
  public async initializeSchema(): Promise<void> {
    // For backward compatibility, still support the old schema.sql approach
    // But prefer using the migration runner for new installations
    try {
      const { MigrationRunner } = await import('./migration-runner.js');
      const migrationRunner = new MigrationRunner();
      await migrationRunner.runMigrations();
    } catch (error) {
      // Fallback to old schema.sql approach
      console.log('Migration runner failed, falling back to schema.sql');

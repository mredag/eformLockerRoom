...
export async function authRoutes(
  ...
  fastify.post("/logout", async (request, reply) => {
    ...
    // Clear all session cookies using bulletproof cookie manager
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (csrfToken && sessionToken) {
        const isValidCsrf = sessionManager.validateCsrfToken(
          sessionToken,
          csrfToken
        );
        if (!isValidCsrf) {
  ...
  fastify.post("/renew", async (request, reply) => {
    ...
    const userAgent = request.headers["user-agent"] || "unknown";
    const renewedSession = sessionManager.renewSession(
      sessionToken,
      ipAddress,
      userAgent
    );
  ...
  fastify.post(
    "/users",
    {
      preHandler: async (request, reply) => {
        ...
        if (
          !PermissionService.hasPermission(user.role, Permission.MANAGE_USERS)
        ) {

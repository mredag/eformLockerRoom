...
function handleWebSocketUpdate(message: any): void {
  try {
    ...
    const lockerElement = document.getElementById(
      `locker-${message.data.kioskId}-${message.data.lockerId}`
    );
    if (lockerElement) {
...
describe("Admin Panel UI E2E", () => {
  ...
  test("should show correct Turkish status translations", async () => {
    ...
    expect(await freeLocker.locator(".locker-state-chip").textContent()).toBe(
      "Boş"
    );

    const ownedLocker = await page.locator('[data-status="Owned"]').first();
    expect(await ownedLocker.locator(".locker-state-chip").textContent()).toBe(
      "Sahipli"
    );
  });
});
